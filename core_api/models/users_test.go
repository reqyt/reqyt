...
func testUserToManyWallets(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"rel_user_wallet\" (\"user_id\", \"wallet_id\") values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"rel_user_wallet\" (\"user_id\", \"wallet_id\") values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManySetOpWallets(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpWallets(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `INTEGER`, `CreatedAt`: `DATETIME`, `UpdatedAt`: `DATETIME`, `DeletedAt`: `DATETIME`, `Name`: `VARCHAR(255)`, `Email`: `VARCHAR(255)`}
	_           = bytes.MinRead
)
